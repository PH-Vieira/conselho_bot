...
export function formatToUTCMinus3(iso) {
  if (!iso) return '';
  try {
    // Some environments may not have dayjs utc plugin loaded reliably.
    // Subtract 3 hours from the ISO timestamp and format as UTC-3 local time.
    return dayjs(iso).subtract(3, 'hour').format('YYYY-MM-DD HH:mm [UTC-3]');
  } catch (e) {
...
export function xpForProposal(openedAtISO, deadlineISO, baseXp = 10) {
  try {
    ...
    // Total duration in seconds (avoid division by zero)
    const totalSec = Math.max(1, Math.abs(deadline.diff(opened, 'second')));
    const now = dayjs();
    // Remaining seconds from now until deadline (clamp to [0, totalSec])
    const remainingSec = Math.max(0, Math.min(totalSec, Math.floor(deadline.diff(now, 'second'))));

    // remainingFraction: 1 when just opened (full time remaining), 0 when expired
    const remainingFraction = remainingSec / totalSec;

    // tuning: base multiplier range and factor from CONFIG.xpScaling if provided
    const cfg = CONFIG.xpScaling || {};
    const minMult = Number.isFinite(Number(cfg.minMult)) ? Number(cfg.minMult) : 0.5; // minimum multiplier when near deadline
    const maxMult = Number.isFinite(Number(cfg.maxMult)) ? Number(cfg.maxMult) : 3.0; // maximum multiplier when just opened

    // Interpolate multiplier linearly by remainingFraction
    let mult = minMult + (maxMult - minMult) * remainingFraction;
